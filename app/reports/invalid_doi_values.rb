...
# Objects with an identifier.value where the identifier type is "doi" or source.value is
# "DOI" (case-insensitive) that does not match the DOI identifier format.
# See https://github.com/sul-dlss/dor-services-app/issues/6248
# Invoke via:
# bin/rails r -e production "InvalidDoiValues.report"
class InvalidDoiValues
  # DOI format regex from https://www.crossref.org/blog/dois-and-matching-regular-expressions/
  DOI_REGEX = '^10\.[0-9]{4,}/[-._;()/:a-zA-Z0-9]+$'

  JSON_PATH = 'strict $.**.identifier[*] ? (' \
              '(@.type like_regex "^doi$" flag "i" || @.source.value like_regex "^doi$" flag "i") ' \
              "&& exists(@.value) && !(@.value like_regex \"#{DOI_REGEX}\"))".freeze

  SQL = <<~SQL.squish.freeze
    SELECT jsonb_path_query(rov.description, '#{JSON_PATH}.value') ->> 0 as value,
