...
class TitleStructuredParts
  ...
  STUCTURED_VALUE_JSON_PATH = "strict $.#{PROPERTY}[*] ? (@.type like_regex \"#{TYPE_REGEX}\")".freeze
  ...
  SQL_QUERY = <<~SQL.squish.freeze
    SELECT
      ...
      form
    FROM "dros",
      jsonb_path_query(dros.description, '#{STUCTURED_VALUE_JSON_PATH}') title_parts,
      jsonb_path_query(dros.description, '#{FORM_JSON_PATH}') form
    WHERE
      jsonb_path_exists(dros.description, '#{STUCTURED_VALUE_JSON_PATH}') AND
      jsonb_path_exists(dros.description, '#{FORM_JSON_PATH}')
  SQL

  def self.report
    puts "item_druid,catkey,collection_druid,collection_name,matching form(s),title parts\n"
  ...
  def self.rows(sql_query)
    ...
    item_druids.map do |item_druid|
      ...
      # each item_row is a hash with keys:   item_druid, catkey, collection_id, title_parts, form
      ...
      [
        ...
        collection_druid,
        "\"#{collection_name}\"",
        item_rows.filter_map { |row| row['form'] }.uniq.join(';'),
        title_parts(item_rows).join(';')
      ].join(',')
    end
  end

  # each row is a hash with keys: item_druid, catkey, collection_id, title_parts, form
