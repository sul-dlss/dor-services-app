...
module Cocina
  module ToFedora
    ...
    class AccessGenerator
      # @param [Nokogiri::Element] root Element that is the root of the access assertions.
      # @param [Cocina::Models::DROAccess, Cocina::Models::Embargo, Cocina::Models::CollectionAccess] access access/rights metadata in Cocina
      ...
      def generate
        ...
        root.add_child(discover_access_node)

        # Add read access node
        root.add_child(read_access_node)

        # Add download access node if needed
        root.add_child(download_access_node) if download_access_node
      end
