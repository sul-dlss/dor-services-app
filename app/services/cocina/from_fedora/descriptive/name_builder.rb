    ...
    class Descriptive
      ...
      class NameBuilder
        ...
        def build_parallel_name(name_node)
          ...
          if name_node['nameTitleGroup'].present? && name_node['altRepGroup'].present?
            matching_title_val = matching_title_value(name_node)
            name_attrs[:appliesTo] = [{ value: matching_title_val }] if matching_title_val
          end
        ...
        def build_name(name_node)
          ...
          #  in a perfect world, it should be sure there is no existing 'usage' attribut of primary on any top level name nodes
          status = name_node['usage']
          ...
          end
          {
            ...
            status: status
          }.compact.merge(common_name(name_node, name_parts)).merge
        end
        ...
        def matching_title_value(name_node)
          ...
          if matching_title_elements.blank?
            notifier.warn("For name '#{name_node.text.strip}', no title matching nameTitleGroup #{desired_name_title_group}.")
            nil
          else
