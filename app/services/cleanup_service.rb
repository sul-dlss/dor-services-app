...
class CleanupService
  ...
  # @param [boolean] revert_metadata if true, descriptive metadata from the last closed version will be restored
  def stop_accessioning(dryrun: false, revert_metadata: false) # rubocop:disable Metrics/PerceivedComplexity, Metrics/CyclomaticComplexity, Metrics/AbcSize, Metrics/MethodLength
    ...
    if repository_object.can_discard_open_version?
      ...
      if repository_object.closed?
        ...
        # Only revert metadata on a reopened object when requested and when the last closed version has Cocina.
        if revert_metadata && repository_object.last_closed_version&.has_cocina?
