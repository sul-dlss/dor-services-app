...
class RefreshDescriptionFromCatalog
  ...
  # @param [string] druid
  # @return [Dry::Monads::Results] returns Failure if there was no refreshable catalog link or barcode, otherwise Success (Result with description_props and mods)
  ...
  # @return [Dry::Monads::Results] Returns Failure if description unchanged (e.g., no refreshable identifiers), otherwise Success (Result with description_props and mods)
  ...
  def run
    ...
    description_props = Cocina::Models::Mapping::FromMods::Description.props(mods: marc_service.mods_ng, druid:, label: cocina_object.label)
