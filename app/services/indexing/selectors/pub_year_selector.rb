...
module Indexing
  module Selectors
    ...
    class PubYearSelector
      ...
      def capture_event
        @capture_event ||= Indexing::Selectors::EventSelector.select(events, 'capture')
      end

      def copyright_event
        @copyright_event ||= Indexing::Selectors::EventSelector.select(events, 'copyright')
      end

      def production_event
        @production_event ||= Indexing::Selectors::EventSelector.select(events, 'production')
      end
    end
  end
end
