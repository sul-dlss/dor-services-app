...
# Manage tags on an object via datastream (legacy method)
class LegacyTagService
  ...
  def validate_and_normalize_tag(tag_str, existing_tag_list)
    ...
    dupe_existing_tag = existing_tag_list.detect { |existing_tag| normalize_tag(existing_tag).casecmp(normalized_tag).zero? }
