...
RSpec.describe 'Show single user version' do
  ...
  before do
    create(:user_version, version: 1, repository_object_version: object.versions.first)
  ...
  it 'returns a 200' do
    get "/v1/objects/#{object.external_identifier}/user_versions/1/solr",
  ...
  context 'with an object that has invalid cocina' do
    before do
      object.versions.first.tap do |version|
    ...
    context 'when validate param is set to false' do
      it 'returns a 200' do
        get "/v1/objects/#{object.external_identifier}/user_versions/1/solr?validate=false",
            headers: { 'Authorization' => "Bearer #{jwt}" }
