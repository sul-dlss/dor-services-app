...
RSpec.describe Cocina::ObjectCreator do
  ...
  before do
    ...
    allow(CocinaObjectStore).to receive(:find).with(apo).and_return(minimal_cocina_admin_policy)
  ...
  context 'when Cocina::Models::DRO is received' do
    ...
    context 'when the type is agreement' do
      let(:request) { build(:dro, id: druid, type: Cocina::Models::ObjectType.agreement) }
  ...
  context 'when geographic is supplied' do
    ...
    let(:request) do
      build(:dro, id: druid, type: Cocina::Models::ObjectType.geo).new(
        geographic: {
          iso19139: geo_xml
        }
      )
