...
RSpec.describe 'MODS originInfo <--> cocina mappings' do
  ...
  context 'when ampersand in placeTerm' do
    # based on zn500ww2119
    # FIXME: normalization problem with xml encoding rather than something here?
    it_behaves_like 'MODS cocina mapping' do
      let(:mods) do
        <<~XML
          <originInfo eventType="publication">
          ...
          event: [
            {
              type: 'publication',
              location: [
                value: 'Leipzig :bVelhagen & Klasing'
              ],
              date: [
                {
                  value: '[between 1940 and 1950?]',
                  type: 'publication'
                }
              ]
            }
  ...
  context 'when square brackets and question mark in date value (?)' do
  ...
  context 'when square brackets and question mark in placeTerm' do
