...
RSpec.describe ReleaseTags::Purl do
  ...
  describe '#released_for' do
    ...
    context 'for targets that are listed on the purl but not in new tag generation' do
      ...
      let(:client) { instance_double(ReleaseTags::PurlClient, fetch: xml) }

      before do
        allow(ReleaseTags::PurlClient).to receive(:new).and_return(client)
      end
