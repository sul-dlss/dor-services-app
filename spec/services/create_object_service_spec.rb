...
RSpec.describe CreateObjectService do
  ...
  describe '#create' do
    ...
    context 'when refreshing from FOLIO MARC with a refreshable FOLIO instance hrid' do
      let(:requested_cocina_object) { build(:request_dro, folio_instance_hrids: ['a999123']) }
      let(:marc_service_marc) do
        ...
        { fields: [
          { '245': {
            ...
            subfields: [
              ...
              { c: 'David A. Black and Joseph Leo III.' }
            ]
          } }
      ...
      before do
        ...
        allow(Catalog::MarcService).to receive(:new).and_return(marc_service_marc)
      end

      it 'adds to description' do
        # debugger
        # expect(store.create(requested_cocina_object).description.title.first.structuredValue.find do |t|
        #   t.type == 'main title'
        # end.value).to eq 'well-grounded Rubyist'
        # expect(store.create(requested_cocina_object).label).to eq 'The well-grounded Rubyist'
        expect(Catalog::MarcService).to have_received(:new).with(folio_instance_hrid: 'a999123')
