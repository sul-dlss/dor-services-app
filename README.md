...
1. Implement a Migrator (`app/services/migrators/`). For the requirements of a Migrator class, see `Migrators::Base` and its subclasses (`Migrators::Exemplar` is a simple examples for illustration; the rest were used for production migrations). Migrators should be unit tested.
...
4. Optionally, verify by running a dry run again.
